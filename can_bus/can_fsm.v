...
module can_fsm (
  ...
  begin
    if (rst)
      ...
      fsm_sample_point <= sample_point && !rx_stall;
  end
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          data_bits_total <= tx_dlc * 8;
          crc_reset <= 1;
          tx_state <= ST_SOF;
        end
      end
      else if (tx_point && !tx_stall)
      begin
        case (tx_state)
          ST_SOF:
          begin
          ...
          begin
            ...
            begin
              ...
              begin
                tx_bit_counter <= data_bits_total - 1;
                tx_state <= ST_DATA;
              end
            end
            else
  ...
  begin
    ...
    begin
      rx_valid <= 0;
      if (rx_state == ST_IDLE && rx_sync_edge)
      begin
        rx_state <= ST_SOF;
      end
      else if (fsm_sample_point)
      begin
        case (rx_state)
          ST_SOF:
          begin
          ...
          ST_ARB:
          begin
            shift_id[rx_bit_counter] <= rx_data_out;
            if (rx_bit_counter == 0)
            begin
              rx_state <= ST_CTRL;
              rx_bit_counter <= 5;
          ...
          begin
            if (rx_bit_counter < 4)
              shift_dlc[rx_bit_counter] <= rx_data_out;
            if (rx_bit_counter == 0)
            begin
              if ({shift_dlc[3:1], rx_data_out} == 0)
              begin
                rx_state <= ST_CRC;
                rx_bit_counter <= 14;
              end
              ...
              begin
                rx_state <= ST_DATA;
                rx_bit_counter <= ({shift_dlc[3:1], rx_data_out} * 8) - 1;
              end
            end
            else
              rx_bit_counter <= rx_bit_counter - 1;
          ...
          ST_DATA:
          begin
            shift_data[rx_bit_counter] <= rx_data_out;
            if (rx_bit_counter == 0)
            begin
              rx_state <= ST_CRC;
              rx_bit_counter <= 14;
