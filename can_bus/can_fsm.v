...
module can_fsm (
    ...
    // 1. ALL DECLARATIONS FIRST (Fixes the syntax cascade)
    reg [3:0] tx_state, rx_state;
    reg fsm_sample_point;
    
    reg [6:0] tx_bit_counter, data_bits_total;
    reg [10:0] latched_id; 
    reg [3:0] latched_dlc; 
    reg [63:0] latched_data;
    
    reg [6:0] rx_bit_counter;
    reg [10:0] shift_id; 
    reg [3:0] shift_dlc; 
    reg [63:0] shift_data;
    ...
    always @(posedge clk) begin
        if (rst) begin 
            tx_state <= ST_IDLE; crc_reset <= 0; tx_bit_counter <= 0; 
            latched_id <= 0; latched_dlc <= 0; latched_data <= 0; data_bits_total <= 0;
        end else begin
            crc_reset <= 0;
    ...
    always @(posedge clk) begin
        if (rst) begin 
            rx_state <= ST_IDLE; rx_valid <= 0; rx_bit_counter <= 0;
            shift_id <= 0; shift_dlc <= 0; shift_data <= 0;
            rx_id <= 0; rx_dlc <= 0; rx_data <= 0;
        end else begin
            rx_valid <= 0;
            if (rx_state == ST_IDLE && rx_sync_edge) begin
                ...
                case (rx_state)
                    ...
                    ST_ARB: begin shift_id[rx_bit_counter] <= rx_data_out;
                            if (rx_bit_counter == 0) begin rx_state <= ST_CTRL; rx_bit_counter <= 5; end else rx_bit_counter <= rx_bit_counter - 1;
                    end
                    ST_CTRL: begin
                    ...
                    ST_DATA: begin shift_data[rx_bit_counter] <= rx_data_out;
                             if (rx_bit_counter == 0) begin rx_state <= ST_CRC; rx_bit_counter <= 14; end else rx_bit_counter <= rx_bit_counter - 1;
                    end
                    ...
                    ST_EOF: begin
                        if (rx_bit_counter == 0) begin 
                            rx_state <= ST_IDLE;
                            rx_id <= shift_id; rx_dlc <= shift_dlc; rx_data <= shift_data; rx_valid <= 1;
                        end 
