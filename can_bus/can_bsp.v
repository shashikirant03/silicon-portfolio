        ...
        if (rst) begin 
            ...
            if (tx_point) begin
                if (!enable_tx_stuffing) begin
                    tx_out <= tx_data_in; tx_ones <= 0; tx_zeros <= 0;
                end else begin
            ...
            if (sample_point) begin
                if (!enable_rx_stuffing) begin
                    rx_data_out <= rx_in; rx_ones <= 0; rx_zeros <= 0;
                end else begin
                    // THE FIX: The stuff bit counts towards the next sequence!
                    if (rx_ones == 5) begin 
                        rx_ones <= 0; rx_zeros <= 1; // It was a 0, count it!
                    end else if (rx_zeros == 5) begin 
                        rx_zeros <= 0; rx_ones <= 1; // It was a 1, count it!
                    end else begin
                        rx_data_out <= rx_in;
