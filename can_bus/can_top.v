...
module can_top (
    ...
    wire enable_tx_stuffing, enable_rx_stuffing; 
    wire [14:0] crc_out;
    wire crc_enable, crc_reset, rx_idle, tx_idle;

    reg q1, q2;
    always @(posedge clk) begin
        if (rst) begin q1 <= 1; q2 <= 1; end 
        else begin q1 <= rx_in; q2 <= q1; end
    end
    ...
    // FIX: Route rx_data_out to CRC when receiving, tx_data_to_bsp when transmitting
    wire crc_data_in = tx_idle ? rx_data_out : tx_data_to_bsp;
    // FIX: Only enable CRC shift during the actual sample point of a bit
    wire gated_crc_en = crc_enable && sample_point;

    can_crc uut_crc (.clk(clk), .rst(crc_reset), .data_in(crc_data_in), .enable(gated_crc_en), .crc_reg(crc_out));
